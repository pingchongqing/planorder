...
<div class="app-container">
  ...
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-button
          v-if="scope.row.paystatus === 'S'"
          @click.native.prevent="refund(scope.row, scope.$index)"
          type="primary"
          :loading="refundLoading[scope.$index]"
          size="small">
          退款
        </el-button>
        <el-button
...
<script>
import { bailBillList, companyList, refundBail, retundDetail } from '@/api/bail'
...
export default {
  ...
  methods: {
    ...
    refund(row, index) {
      this.$set(this.refundLoading, index, true)
      refundBail({ ticketno: row.ticketno }).then(
        res => {
          this.$message({
            message: res.message,
            type: 'success'
      ...
      ).catch(_ => { this.$set(this.refundLoading, index, false) })
    },
    handelDialogConfirm() {
